...
pub fn configure(radio: &mut Registers, board: &Board) -> Result<()> {
    ...
    radio.PINFUNCIRQ().write(      PFIRQ { mode: board.irq.mode.into(),    flags: board.irq.into()})?;
    radio.PINFUNCANTSEL().write(PFAntSel { mode: board.antsel.mode.into(), flags: board.antsel.into()})?;
...
pub fn configure_synth(radio: &mut Registers, board: &Board, synth: &Synthesizer) -> Result<()> {
    ...
    radio.FREQA().write(to_freq(synth.freq_a, board.xtal.freq))?;
    radio.FREQB().write(to_freq(synth.freq_b, board.xtal.freq))?;
...
pub enum Modulation {
    ...
    OQPSK{ ramp: u8 },
    MSK  { ramp: SlowRamp },
    GMSK { ramp: SlowRamp, bt: BT },
    FSK  { deviation: u64, ramp: SlowRamp },
    GFSK { deviation: u64, ramp: SlowRamp, bt: BT },
    FSK4 { deviation: u64, ramp: SlowRamp, nosync: bool },
    AFSK { deviation: u64, ramp: SlowRamp }, // Note: different formula than FSK
    FM   { deviation: u64, ramp: SlowRamp, adc: ADCcfg },
//  AM?
}
...
pub fn configure_channel(radio: &mut Registers, board: &Board, parameters: &ChannelParameters) -> Result<()> {
...
pub fn configure_tx(radio: &mut Registers, board: &Board, channel: &ChannelParameters, parameters: &TXParameters) -> Result<()> {
    // MODULATION,
    // FSKDEV,
    // MODCFGA,
    // MODCFGF,
