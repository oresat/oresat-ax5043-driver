...
fn print_powerstate(radio: &mut Registers, state: &mut State) -> io::Result<()> {
...
fn main() -> io::Result<()> {
    ...
    let mut state = State {
        stat: PowStat::default(),
        sstat: PowStat::default(),
        irq: IRQ::default(),
    };
    ...
    radio.PWRMODE.write(PwrMode { mode: PwrModes::POWEROFF, flags: PwrFlags::REFEN})?;
    print_powerstate(&mut radio, &mut state)?;
...
/*
    ...
    println!("FREQDEV13:      {:?}", radio.FREQDEV13.read()?);
